...
extract_local_package() {
  ...
  info "Successfully extracted binary from local package"
   ...
   COMPLETIONS_SRC=$(find "$TEMP_DIR" -name "completions" -type d | head -n 1)
  if [ -n "$COMPLETIONS_SRC" ]; then
    info "Copying completion scripts from local package..."
    cp -r "$COMPLETIONS_SRC" "$KMDO_HOME/" || warn "Failed to copy completion scripts (ignored)"
  else
    info "No completions directory found in local package (skip copying)"
  fi
}
